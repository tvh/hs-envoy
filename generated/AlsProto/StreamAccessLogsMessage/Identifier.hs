{-# LANGUAGE BangPatterns, DataKinds, DeriveDataTypeable, DeriveGeneric, FlexibleInstances, MultiParamTypeClasses, OverloadedStrings
 #-}
{-# OPTIONS_GHC  -w #-}
module AlsProto.StreamAccessLogsMessage.Identifier (Identifier(..)) where
import Prelude ((+), (/), (++), (.))
...
data Identifier = Identifier{node :: !(P'.Maybe BaseProto.Node), log_name :: !(P'.Maybe P'.Utf8)}
        ...
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{node = P'.mergeAppend (node old'Self) (Prelude'.Just new'Field)})
                    (P'.wireGet 11)
             18 -> Prelude'.fmap (\ !new'Field -> old'Self{log_name = Prelude'.Just new'Field}) (P'.wireGet 9)
...
instance P'.ReflectDescriptor Identifier where
  ...
  reflectDescriptorInfo _
   = Prelude'.read
      "DescriptorInfo {descName = ProtoName {protobufName = FIName \".envoy.service.accesslog.v3.StreamAccessLogsMessage.Identifier\", haskellPrefix = [], parentModule = [MName \"AlsProto\",MName \"StreamAccessLogsMessage\"], baseName = MName \"Identifier\"}, descFilePath = [\"AlsProto\",\"StreamAccessLogsMessage\",\"Identifier.hs\"], isGroup = False, fields = fromList [FieldInfo {fieldName = ProtoFName {protobufName' = FIName \".envoy.service.accesslog.v3.StreamAccessLogsMessage.Identifier.node\", haskellPrefix' = [], parentModule' = [MName \"AlsProto\",MName \"StreamAccessLogsMessage\",MName \"Identifier\"], baseName' = FName \"node\", baseNamePrefix' = \"\"}, fieldNumber = FieldId {getFieldId = 1}, wireTag = WireTag {getWireTag = 10}, packedTag = Nothing, wireTagLength = 1, isPacked = False, isRequired = False, canRepeat = False, mightPack = False, typeCode = FieldType {getFieldType = 11}, typeName = Just (ProtoName {protobufName = FIName \".envoy.config.core.v3.Node\", haskellPrefix = [], parentModule = [MName \"BaseProto\"], baseName = MName \"Node\"}), hsRawDefault = Nothing, hsDefault = Nothing, isMapField = False, mapKeyVal = Nothing},FieldInfo {fieldName = ProtoFName {protobufName' = FIName \".envoy.service.accesslog.v3.StreamAccessLogsMessage.Identifier.log_name\", haskellPrefix' = [], parentModule' = [MName \"AlsProto\",MName \"StreamAccessLogsMessage\",MName \"Identifier\"], baseName' = FName \"log_name\", baseNamePrefix' = \"\"}, fieldNumber = FieldId {getFieldId = 2}, wireTag = WireTag {getWireTag = 18}, packedTag = Nothing, wireTagLength = 1, isPacked = False, isRequired = False, canRepeat = False, mightPack = False, typeCode = FieldType {getFieldType = 9}, typeName = Nothing, hsRawDefault = Nothing, hsDefault = Nothing, isMapField = False, mapKeyVal = Nothing}], descOneofs = fromList [], keys = fromList [], extRanges = [], knownKeys = fromList [], storeUnknown = False, lazyFields = False, makeLenses = False, jsonInstances = False, mapEntry = False}"
...
instance P'.TextMsg Identifier where
  textPut msg
   = do
       ...
       P'.tellT "log_name" (log_name msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'node, parse'log_name]) P'.spaces
       Prelude'.return (Prelude'.foldl' (\ v f -> f v) P'.defaultValue mods)
    where
        parse'node = Prelude'.fmap (\ v o -> o{node = v}) (P'.try (P'.getT "node"))
        parse'log_name = Prelude'.fmap (\ v o -> o{log_name = v}) (P'.try (P'.getT "log_name"))